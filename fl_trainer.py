...
def data_generater(trainer_id,inner_step,batch_size,count_by_step):
	train_file = open("./data/train/all_data_%d_niid_0_keep_0_train_9.json" % trainer_id,'r')
	test_file = open("./data/test/all_data_%d_niid_0_keep_0_test_9.json" % trainer_id, 'r')
...
count_by_step = True
if count_by_step:
	output_folder = "model_node_step1%d" % trainer_id
else: 
...
while not trainer.stop():
    ...
    # print("acc:%.3f" % (acc[0]))
        ...
        trainer.run_with_epoch(train_reader,feeder,fetch=["accuracy_0.tmp_0"],e=1) 
